...
const getSchemaExtension = ({
  ...
  magicAuthLink,
  gqlNames
})].filter(x => x).reduce((s, extension) => merge.mergeSchemas(_objectSpread({
  schemas: [s]
}, extension)), schema);
/**
 * createAuth function
...
function createAuth({
  ...
  const additionalListFields = {
    [`${secretField}ResetToken`]: fields.password(_objectSpread({}, fieldConfig)),
    [`${secretField}ResetIssuedAt`]: fields.timestamp(_objectSpread({}, fieldConfig)),
    [`${secretField}ResetRedeemedAt`]: fields.timestamp(_objectSpread({}, fieldConfig)),
    [`magicAuthToken`]: fields.password(_objectSpread({}, fieldConfig)),
    [`magicAuthIssuedAt`]: fields.timestamp(_objectSpread({}, fieldConfig)),
    [`magicAuthRedeemedAt`]: fields.timestamp(_objectSpread({}, fieldConfig))
  };
  /**
   * adminPageMiddleware
   *
  ...
  const adminPageMiddleware = async ({
    ...
    const pathname = url__default['default'].parse(req.url).pathname;
