...
var initConfig = require('../../dist/initConfig-34e7aba3.cjs.dev.js');
...
const createExpressServer = async (config, graphQLSchema, createContext, dev, projectAdminPath) => {
  ...
  server.use(await system.createAdminUIServer(config.ui, createContext, dev, projectAdminPath, sessionImplementation));
  return server;
};
...
const dev = async ({
  ...
  const initKeystone = async () => {
    const {
      ...
      graphQLSchema,
      createContext
    } = initConfig.createSystem(config, dotKeystonePath, script);
...
const start = async ({
  ...
  const {
    ...
    graphQLSchema,
    createContext
  } = initConfig.createSystem(config, dotKeystonePath, 'start');
  console.log('✨ Connecting to the database');
  await keystone.connect({
    context: createContext({
      skipAccessControl: true
...
async function build({
  ...
  const {
    keystone,
    graphQLSchema
  } = initConfig.createSystem(config, dotKeystonePath, 'build');
  console.log('✨ Generating graphQL schema');
  await saveSchemaAndTypes(graphQLSchema, keystone, dotKeystonePath);
  console.log('✨ Generating Admin UI code');
  await system.generateAdminUI(config, graphQLSchema, keystone, projectAdminPath);
...
const deploy = async ({
  ...
  const config = initConfig.initConfig(requireSource(CONFIG_PATH).default);
  const keystone = initConfig.createKeystone(config, dotKeystonePath, 'deploy');
...
const generate = async ({
  ...
  const {
    ...
    graphQLSchema,
    createContext
  } = initConfig.createSystem(config, dotKeystonePath, 'generate');
  console.log('✨ Generating graphQL schema');
  await saveSchemaAndTypes(graphQLSchema, keystone, dotKeystonePath);
