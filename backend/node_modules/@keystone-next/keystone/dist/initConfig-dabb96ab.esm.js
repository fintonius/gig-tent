...
import { GraphQLObjectType, GraphQLNonNull, GraphQLList, parse, execute } from 'graphql';
import { mergeSchemas } from '@graphql-tools/merge';
import { mapSchema } from '@graphql-tools/utils';
import { makeExecutableSchema } from '@graphql-tools/schema';
import { getAdminMetaSchema } from '@keystone-next/admin-ui/system';
import { g as gql } from './schema-15c288ef.esm.js';
import { getItem, getItems, createItem, createItems, updateItem, updateItems, deleteItem, deleteItems } from '@keystonejs/server-side-graphql-client';
...
import { mongoId, autoIncrement } from '@keystone-next/fields';
...
function createGraphQLSchema(config, keystone) {
  ...
  graphQLSchema = mapSchema(graphQLSchema, {
    'MapperKind.OBJECT_TYPE'(type) {
      if (config.lists[type.name] !== undefined && config.lists[type.name].fields._label_ === undefined) {
        ...
        return new GraphQLObjectType(_objectSpread({
          fields
        }, objectTypeConfig));
      }
  ...
  graphQLSchema = mergeSchemas(_objectSpread({
    schemas: [graphQLSchema]
  }, getAdminMetaSchema({
    keystone,
    config
...
function createKeystone(config, dotKeystonePath, script) {
  ...
  if (db.adapter === 'knex') {
    ...
    adapter = new MongooseAdapter(_objectSpread({
      mongoUri: db.url
    }, db.mongooseOptions));
  } else if (db.adapter === 'prisma_postgresql') {
    adapter = new PrismaAdapter(_objectSpread({
...
export { createKeystone as a, createSystem as c, initConfig as i };
