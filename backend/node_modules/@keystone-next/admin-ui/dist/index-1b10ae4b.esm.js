...
import { s as staticAdminMetaQuery } from './admin-meta-graphql-c0552487.esm.js';
import crypto from 'crypto';
import Path, { dirname, join } from 'path';

const viewHash = views => `view${crypto.createHash('sha256').update(views).digest('hex').slice(0, 8)}`;
...
const appTemplate = (config, graphQLSchema, {
  ...
  for (const views of allViews) {
    const viewPath = Path.isAbsolute(views) ? Path.relative(Path.join(projectAdminPath, 'pages'), views) : views; // Convert filesystem path separator to the `/` expected in JS imports
...
const pkgDir = dirname(require.resolve('@keystone-next/admin-ui/package.json'));
const writeAdminFiles = (config, graphQLSchema, keystone, configFileExists, projectAdminPath) => [...['next.config.js', 'tsconfig.json'].map(outputPath => ({
  mode: 'copy',
  inputPath: join(pkgDir, 'static', outputPath),
  outputPath
})), {
  mode: 'write',
...
export { viewHash as v, writeAdminFiles as w };
