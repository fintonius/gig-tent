...
import { w as writeAdminFiles, v as viewHash } from '../../dist/index-1b10ae4b.esm.js';
import url from 'url';
...
async function writeAdminFile(file, projectAdminPath) {
  ...
  if (file.mode === 'copy') {
    ...
    await fs.ensureDir(Path.dirname(outputFilename)); // TODO: should we use copyFile or copy?
...
const generateAdminUI = async (config, graphQLSchema, keystone, projectAdminPath) => {
  ...
  await fs.remove(projectAdminPath); // Write out the files configured by the user
  ...
  const adminFiles = writeAdminFiles(config, graphQLSchema, keystone, configFileExists, projectAdminPath);
  const baseFiles = adminFiles.filter(x => !uniqueFiles.has(Path.normalize(x.outputPath))); // FIXME: This path corresponds to the hardcoded value in CONFIG_PATH

  const configModule = Path.join(projectAdminPath, '..', '..', 'keystone');
  const outputDir = Path.join('pages', 'api');
  const pathToConfig = Path.relative(Path.join(projectAdminPath, outputDir), configModule); // this should always exist, the user should not be able to override it.
  ...
  await Promise.all(files.map(async filename => {
    ...
    const path = Path.relative(Path.dirname(outputFilename), Path.join(userPagesDir, filename)); // Convert filesystem path separator to the `/` expected in JS imports
  ...
  Object.keys(lists).forEach(key => {
    ...
    for (const fieldKey of Object.keys(listConfig.fields).filter(path => {
      ...
      adminMetaRoot.listsByKey[key].fields.push({
        ...
        customViewsHash: ((_field$config$ui = field.config.ui) === null || _field$config$ui === void 0 ? void 0 : _field$config$ui.views) === undefined ? null : viewHash(field.config.ui.views),
...
export { buildAdminUI, createAdminUIServer, generateAdminUI, getAdminMetaSchema };
