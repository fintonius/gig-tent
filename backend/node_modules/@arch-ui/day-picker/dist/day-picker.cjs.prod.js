...
var _extends = require("@babel/runtime/helpers/extends"), core = require("@emotion/core"), styled = require("@emotion/styled"), dateFns = require("date-fns"), reactWindow = require("react-window"), octiconsReact = require("@primer/octicons-react"), React = require("react"), theme = require("@arch-ui/theme"), typography = require("@arch-ui/typography");

require("intersection-observer");

var input = require("@arch-ui/input"), Select = require("@arch-ui/select"), _defineProperty = require("@babel/runtime/helpers/defineProperty"), _objectWithoutProperties = require("@babel/runtime/helpers/objectWithoutProperties"), chrono = require("chrono-node"), moment = require("moment");
...
var _extends__default = _interopDefault(_extends), styled__default = _interopDefault(styled), React__default = _interopDefault(React), Select__default = _interopDefault(Select), _defineProperty__default = _interopDefault(_defineProperty), _objectWithoutProperties__default = _interopDefault(_objectWithoutProperties), chrono__default = _interopDefault(chrono), moment__default = _interopDefault(moment);
...
}), WeekRow = styled__default.default.div({
  display: "flex"
}), WeekLabels = styled__default.default(WeekRow)({
  color: theme.colors.N40,
  fontSize: "0.65rem",
  fontWeight: 500,
  textTransform: "uppercase"
}), Day = styled__default.default.div(({disabled: disabled, isInteractive: isInteractive, isSelected: isSelected, isToday: isToday}) => {
  let textColor;
...
}), TodayMarker = styled__default.default.div(({isSelected: isSelected}) => ({
...
}, year)))), Wrapper = styled__default.default.div({
  fontSize: "0.85rem"
}), Header = styled__default.default.div({
  alignItems: "center",
  display: "flex",
  justifyContent: "space-between"
}), HeaderButton = props => core.jsx("button", _extends__default.default({
  type: "button",
  css: {
    background: "none",
    borderRadius: theme.borderRadius,
...
const TextDayPicker = React.forwardRef((_ref, ref) => {
  let {date: date = "", onChange: onChange, format: displayFormat = "do MMMM yyyy"} = _ref, props = _objectWithoutProperties__default.default(_ref, [ "date", "onChange", "format" ]);
  ...
  React.useEffect(() => {
    const parsedDate = chrono__default.default.parseDate(value.raw), isoDate = parsedDate ? dateFns.formatISO(parsedDate, {
      representation: "date"
    }) : null;
    onChange(isoDate), parseCache.current = isoDate;
  }, [ value.raw ]);
  return React__default.default.createElement(input.Input, _extends__default.default({
    ref: ref,
...
let TextDayTimePicker = _ref => {
  let {date: date, onChange: onChange} = _ref, props = _objectWithoutProperties__default.default(_ref, [ "date", "onChange" ]), [value, setValue] = React.useState(""), ref = React.useRef(null);
  return React.useEffect(() => {
    setValue(formatDateTime(date));
  }, [ date ]), core.jsx(input.Input, _extends__default.default({
    value: value,
    ref: ref,
...
function parsedDateToMoment(parsedDate) {
  let dateMoment = moment__default.default(), currentTimezoneOffset = moment__default.default().utcOffset(), {start: start} = parsedDate;
