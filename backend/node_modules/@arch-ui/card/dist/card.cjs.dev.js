'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var _extends = require('@babel/runtime/helpers/extends');
var _objectWithoutProperties = require('@babel/runtime/helpers/objectWithoutProperties');
var core = require('@emotion/core');
var theme = require('@arch-ui/theme');

function _interopDefault (e) { return e && e.__esModule ? e : { 'default': e }; }

var _extends__default = /*#__PURE__*/_interopDefault(_extends);
var _objectWithoutProperties__default = /*#__PURE__*/_interopDefault(_objectWithoutProperties);

// Card
// ==============================

const Card = (_ref) => {
  let {
    as: Tag = 'div',
    elevation = 0,
    isInteractive,
    isPadded = true
  } = _ref,
      props = _objectWithoutProperties__default['default'](_ref, ["as", "elevation", "isInteractive", "isPadded"]);

  const shadow = theme.shadows[elevation];
  return core.jsx(Tag, _extends__default['default']({
    css: {
      background: 'white',
      borderRadius: theme.borderRadius,
      boxShadow: shadow,
      padding: isPadded ? theme.gridSize * 2 : null,
      transition: `box-shadow 111ms ease-out, transform 111ms ease-out`,
      ':hover': isInteractive ? {
        boxShadow: theme.shadows[elevation + 1],
        transform: 'translateY(-2px)'
      } : null,
      ':active': isInteractive ? {
        boxShadow: shadow,
        transform: 'none'
      } : null
    }
  }, props));
}; // ==============================
// Canvas
// ==============================

const Canvas = (_ref2) => {
  let {
    as: Tag = 'div',
    isPadded = true
  } = _ref2,
      props = _objectWithoutProperties__default['default'](_ref2, ["as", "isPadded"]);

  return core.jsx(Tag, _extends__default['default']({
    css: {
      background: `rgba(9, 30, 66, 0.04)`,
      borderRadius: theme.borderRadius * 2,
      padding: isPadded ? theme.gridSize : null
    }
  }, props));
};

exports.Canvas = Canvas;
exports.Card = Card;
